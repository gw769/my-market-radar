...
export default function Analyze() {
  ...
  const [platforms, setPlatforms] = useState(["shopee", "lazada"]);
  ...
  const submit = async (event: FormEvent) => {
    ...
    setBusy(true);
    try {
      const res = await apiPost<any>("/keywords", { keyword, platforms, results_limit: 20, tracking_enabled: tracking, daily_time: "20:00", timezone: "Asia/Kuala_Lumpur" });
      navigate(`/tracking?run_id=${res.run.id}`);
    } catch (err: any) { setError(err.message || "创建分析失败"); } finally { setBusy(false); }
  };
  return <div className="page-stack">
    <section className="hero-split">
      <div><span className="eyebrow">KEYWORD-FIRST WORKFLOW</span><h2>不用商品编号。<br /><em>说出你想卖什么。</em></h2><p>系统会在马来西亚两个主流平台寻找公开竞品，并把不可比较的口径分开处理。</p></div>
      <div className="hero-signal"><Zap /><strong>40</strong><span>默认样本上限<br />20 × 2 平台</span></div>
    </section>
    <form onSubmit={submit} className="analysis-console panel">
      ...
      <label className="tracking-choice"><input type="checkbox" checked={tracking} onChange={(e) => setTracking(e.target.checked)} /><Clock3 /><div><strong>每日 20:00 自动跟踪</strong><span>保存价格、公开已售、评论与搜索排名变化</span></div></label>
      <div className="notice-row"><ShieldAlert size={17} /><span>平台要求验证时任务会暂停，由你手动完成验证；系统不会自动绕过。</span></div>
      {error && <div className="error-box">{error}</div>}
      <button className="primary-button analyze-button" disabled={busy}>{busy ? "正在创建扫描…" : "开始双平台分析"}<Search size={18} /></button>
    </form>
  </div>;
}
