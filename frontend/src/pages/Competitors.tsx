...
export default function Competitors() {
  ...
  useEffect(() => {
    apiGet<any>("/keywords").then((r) => {
      const list = r.data || [];
      setKeywords(list);
      if (list[0]) setKeywordId(list[0].id);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    apiGet<any>(`/runs/${stableRunId}/items`).then((r) => setItems(r.data || []));
  }, [stableRunId]);
  ...
  return <div className="page-stack">
    ...
    {showingOlderResult && <div className="info-box">当前任务状态：{activeRun?.status}。下表暂时保留上一份稳定竞品快照。</div>}
