...
class CalibrationTests(unittest.TestCase):
    ...
    def test_combined_analysis_uses_confidence_weighting_and_updates_prose(self):
        analysis = {
            ...
            "recommendations": ["“water bottle”当前公开数据机会分为 60，结论：谨慎观察。"],
            "methodology": "基础方法。",
        }
        ...
        self.assertIn("校准后的机会分为 70.0", calibrated["recommendations"][0])
        self.assertIn("结论：建议尝试", calibrated["recommendations"][0])
        self.assertFalse(any("机会分为 60" in text for text in calibrated["recommendations"]))
        self.assertIn("跨平台总分按各平台数据完整度加权", calibrated["methodology"])
    ...
    def test_small_single_platform_segment_is_shrunk_toward_neutral(self):
        ...
        self.assertFalse(any("当前排序最高" in text for text in calibrated["recommendations"]))
