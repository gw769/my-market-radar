...
### 3.3 可选的 Shopdora Shopee 插件增强

如果用户的普通 Chrome 已安装并正常加载 Shopdora，系统会在不改变原始 Shopee 字段的前提下，额外读取它注入到商品卡中的增强字段：

- 卖家、店铺类型、品牌和类目路径；
- 类目月销量排名、上架日期和上架天数；
- 点赞数；
- 近 30 日销量及增长率；
- 近 30 日销售额、总销量估算和 GMV 估算。

这些字段始终标记为“Shopdora · 第三方估算”，不会覆盖 Shopee 页面公开已售数，也不会参与确定性机会分。免费套餐当前可能只增强每页部分商品，所以报告会同时显示“插件增强样本数 / Shopee 快照样本数”。插件未安装、未登录或加载失败时，Shopee 和 Lazada 的基础采集仍可正常完成。

当前 Lazada 没有接入同类插件增强，继续使用 Lazada 搜索页公开字段。Shopdora 的“综合评价、综合分析、流量分析、市场分析”按钮属于插件自身界面；系统不会把未验证的付费分析结果当作事实，也不会把“点赞数”误称为“评论数”。
...
如果本轮采到了 Shopdora 增强字段，AI 还会收到经过聚合且明确标注为第三方估算的覆盖率、中位数和类目摘要。AI 可以据此提出“下一步应该核验什么”，但不能把插件估算写成平台真实销量，也不能用它修改机会分。
...
- AI 请求设置 `store=false`；
- AI 只读取聚合后的公开字段，以及明确标注来源和估算属性的 Shopdora 聚合摘要；
- 浏览器登录态留在用户 Chrome 内；
- 验证码只允许人工处理；
...
- 真实 GMV、真实月销量或店铺后台转化率；Shopdora 显示的对应数字仍属于第三方估算；
- 广告实际花费；
- 采购、物流、退货、税费和净利润；
- 平台完整市场份额；
- 某个商品未来一定增长；
